...
$api->version('v1', function (Router $api) {
    ...
    $api->group(['middleware' => 'cors'], function (Router $api) {
        ...
        $api->group(['prefix' => 'plans'], function(Router $api) {
            $api->get('/', 'App\\Api\\V1\\Controllers\\PlansController@index')->name('plans');
        });

        $api->group(['prefix' => 'subscriptions'], function(Router $api) {
            Route::post('/', 'App\\Api\\V1\\Controllers\\SubscriptionsController@store')->name('subscription.store');
        });
